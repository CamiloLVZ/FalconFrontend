...
export const ManageReservationPage = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch({
        type: "SEARCH_ERROR",
        message: getApiErrorMessage(err, "No se pudo obtener la reserva."),
      });
    }
  };
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const updated = await cancelReservation(
        state.reservation.number,
        state.reservation.contactEmail,
      );
      dispatch({ type: "CANCEL_SUCCESS", reservation: updated });
    } catch (err) {
      dispatch({
        type: "CANCEL_ERROR",
        message: getApiErrorMessage(err, "No se pudo cancelar la reserva."),
      });
    }
  };
  ...
  const checkInAvailable =
    state.reservation?.flight?.status === "CHECK_IN_AVAILABLE";
  ...
  if (state.view === "result" && state.reservation) {
    ...
    const hasActivePassengers = reservation.passengers.some(
      (p) => p.status === "RESERVED" || p.status === "CHECKED_IN",
    );
        ...
        <div className="w-full max-w-2xl">
          <div className="flex items-center justify-between mb-6">
            <h1
              ...
              data-testid="manage-details-reservation-number-label"
            >
              Reserva {reservation.number}
            </h1>
            <button
              type="button"
              data-testid="manage-details-new-search-button"
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-4">
            <div className="flex items-center justify-between mb-4">
              <span
                ...
                data-testid="manage-details-flight-number-label"
              >
                {flight.flightNumber}
              </span>
              <span
                className={`text-xs px-3 py-1 rounded-full font-medium ${statusColor[reservation.status] || "bg-gray-100 text-gray-600"}`}
              >
            ...
            <div className="space-y-3">
              {reservation.passengers.map((pr) => (
                <div
                  ...
                  data-testid="manage-details-passenger-item"
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {pr.seatLabel && `${pr.seatLabel} · `}
                      {pr.seatClass === "FIRST_CLASS"
                        ? "Primera clase"
                        : "Económico"}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {checkInAvailable && pr.status === "RESERVED" ? (
                      <button
                        ...
                        onClick={() =>
                          handleGoToCheckIn(
                            pr.passenger.identificationNumber,
                            pr.passenger.nationalityIsoCode,
                          )
                        }
                      ...
                      <span
                        className={`text-xs px-3 py-1 rounded-full font-medium ${statusColor[pr.status] || "bg-gray-100 text-gray-600"}`}
                      >
                        {passengerStatusLabel[pr.status] || pr.status}
                      </span>
                    )}
                    {(pr.status === "CHECKED_IN" ||
                      pr.status === "BOARDED") && (
                      <button
                        type="button"
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              {!state.cancelConfirm ? (
                <button
                  type="button"
                  onClick={() =>
                    dispatch({ type: "SET_CANCEL_CONFIRM", value: true })
                  }
                ...
                <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
                  ...
                  <div className="flex gap-3">
                    <button
                      type="button"
                      onClick={() =>
                        dispatch({ type: "SET_CANCEL_CONFIRM", value: false })
                      }
    ...
    <div className="flex flex-col items-center bg-blue-50 min-h-screen px-4 py-10">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold" data-testid="manage-title">
            Gestionar reserva
          </h1>
          <p className="text-gray-500 mt-2">
            Ingresa los datos de tu reserva para consultarla
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <form onSubmit={handleSearch} className="space-y-4">
            <div>
              <label
                htmlFor="manage-reservationNumber"
              ...
              <input
                ...
                data-testid="manage-reservation-number-input"
            ...
            <div>
              ...
              <input
                ...
                data-testid="manage-contact-email-input"
            ...
            <button
              type="submit"
              data-testid="manage-search-button"
