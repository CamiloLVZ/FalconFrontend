...
import type {
  ...
  PassengerGender,
} from "../admin/features/passengers/types/passengerTypes";
import type {
  Reservation,
  ReservationStatus,
...
const profileFormReducer: Reducer<ProfileFormState, ProfileFormAction> = (
  state,
  action,
) => {
  switch (action.type) {
...
const reservationsReducer: Reducer<ReservationsState, ReservationsAction> = (
  state,
  action,
) => {
  switch (action.type) {
...
const cancelingReducer: Reducer<CancelingState, CancelingAction> = (
  state,
  action,
) => {
  switch (action.type) {
...
const UserBanner = ({
  ...
  <div
    ...
    className="bg-gradient-to-r from-[#0B1C2C] to-[#1e3a5f] rounded-2xl p-6 sm:p-8 text-white shadow-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
  >
    <div>
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 data-testid="user-profile-name" className="text-2xl font-bold">
            {displayName}
          </h1>
          <p
  ...
  <div className="space-y-6">
    ...
    <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      <h2 className="text-lg font-bold text-gray-800">
        Historial de Vuelos y Reservas
      </h2>
    ...
    {loading ? (
      ...
      <div className="bg-white rounded-2xl p-12 text-center text-gray-500 shadow-sm border border-gray-100 space-y-4">
        ...
        <h3 className="text-lg font-bold text-gray-800">
          No tienes reservas {statusFilter ? "con este estado" : "aún"}
        </h3>
        <p className="text-sm text-gray-500 max-w-md mx-auto">
          ¿Buscas un viaje? Realiza una búsqueda de vuelos e inicia tu próxima
          aventura.
        </p>
        <Link
          to="/"
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800 text-sm">
                    {new Date(res.flight.departureDateTime).toLocaleString(
                      "es-CO",
                      {
                        dateStyle: "medium",
                        timeStyle: "short",
              ...
              <div className="flex items-center gap-1.5 text-[11px] text-slate-500 italic">
                ...
                <span>
                  El abordaje comienza 40 minutos antes de la salida del vuelo.
                </span>
              </div>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          <span className="font-bold text-blue-600">
                            {p.seatLabel || "Sin asignar"}
                          </span>{" "}
                          (
                          {p.seatClass === "FIRST_CLASS"
                            ? "Primera Clase"
                            : "Económica"}
                          )
                        </p>
                        ...
                        {res.status === "RESERVED" &&
                          p.status === "RESERVED" && (
                            <button
                              ...
                              disabled={
                                cancelingPassengerId ===
                                `${res.number}-${p.passenger.identificationNumber}`
                              }
                  ...
                  <>
                    {checkInOpen ? (
                      ...
                      <div className="flex flex-col items-end sm:items-start gap-1">
                        ...
                        <span className="text-[11px] text-slate-500 font-medium italic">
                          * El check-in está disponible únicamente entre 24 y 1
                    ...
                    {cancelAllowed && (
                      <button
                        type="button"
                        onClick={() =>
                          onCancelReservation(res.number, res.contactEmail)
                        }
...
const ProfileDetailsView = ({ profile }: { profile: Passenger | null }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl">
    ...
    <div>
      ...
      <p className="font-semibold text-gray-800">
        {profile?.passportNumber || "No registrado"}
      </p>
    </div>
...
const ProfileForm = ({
  ...
  <form onSubmit={onSubmit} className="space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div>
        <label
          htmlFor="profile-identificationNumber"
          className="block text-xs font-semibold text-gray-700 mb-1"
        >
      ...
      <div>
        <label
          htmlFor="profile-nationalityIsoCode"
          className="block text-xs font-semibold text-gray-700 mb-1"
        >
          País de Emisión / Nacionalidad *
        </label>
        <select
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div>
        <label
          htmlFor="profile-passportNumber"
          className="block text-xs font-semibold text-gray-700 mb-1"
        >
          Pasaporte (Opcional)
        </label>
  ...
  <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-200/80 max-w-3xl mx-auto space-y-6">
    ...
    {!hasProfile && !loading && (
      <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl text-sm flex items-center gap-2">
        ...
        <span>
          Aún no has registrado tu perfil de pasajero. Completa los datos a
          continuación para vincular tu información personal a tu usuario.
        </span>
      </div>
    )}
...
export const UserProfilePage = () => {
  ...
  const [reservations, reservationsDispatch] = useReducer(
    reservationsReducer,
    initialReservationsState,
  );
  const [canceling, cancelingDispatch] = useReducer(
    cancelingReducer,
    initialCancelingState,
  );
    ...
    try {
      ...
      if (profile.hasProfile) {
        ...
        profileDispatch({
          type: "PROFILE_SAVED",
          profile: created,
          message: "Perfil de pasajero creado exitosamente.",
        });
      }
    } catch (err) {
      if (axios.isAxiosError<ApiErrorResponse>(err)) {
        profileDispatch({
          ...
          message: err.response?.data.message ?? "Error al guardar el perfil.",
        });
      } else {
        profileDispatch({
          type: "PROFILE_SAVE_ERROR",
  ...
  const handleCancelPassenger = async (
    ...
    try {
      ...
      await cancelMyPassengerReservation(
        resNumber,
        contactEmail,
        identificationNumber,
        countryIsoCode,
      ...
      alert(
        "No se pudo cancelar el pasajero. Verifica que la reserva esté activa.",
      );
    } finally {
  ...
  const handleCancelReservation = async (resNumber: string, email: string) => {
    if (
      !window.confirm(
        `¿Estás seguro de que deseas cancelar la reserva ${resNumber}?`,
      )
    ) {
      return;
    }
        ...
        {activeTab === "profile" && (
          <ProfileTab
            ...
            onGenderChange={(value) =>
              formDispatch({ type: "SET_GENDER", value })
            }
