...
export const Sidebar = ({ mobileOpen, onMobileClose }: SidebarProps) => {
  const sidebar = (
    <aside className="flex h-full w-[310px] flex-col bg-[#071c33] text-slate-100 shadow-2xl">
      ...
      <div className="px-7 pb-6 pt-7" data-testid="admin-sidebar-header">
      ...
      <nav className="flex-1 space-y-3 overflow-y-auto px-2" data-testid="admin-sidebar-nav">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            data-testid={`admin-nav-${item.path.replace("/admin/", "")}`}
