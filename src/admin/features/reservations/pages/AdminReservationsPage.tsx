...
import { getReservationsByFlight, cancelReservation } from "../services/reservationService";
...
export const AdminReservationsPage = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getReservationsByFlight(Number(flightId));
  ...
  const handleCancelReservation = async () => {
    ...
    if (!window.confirm(`¿Estás seguro de que deseas cancelar la reserva ${selectedReservation.number}?`)) {
      return;
    }
    ...
    try {
      ...
      const updatedReservation = await cancelReservation(selectedReservation.number);
      
      setReservations((prev) => 
        prev.map((r) => (r.number === updatedReservation.number ? updatedReservation : r))
      );
      setIsDrawerOpen(false);
    } catch (err) {
      setActionError(getApiErrorMessage(err, "No se pudo cancelar la reserva."));
    } finally {
  ...
  return (
    <section className="min-h-[calc(100vh-136px)]">
      ...
      <div className="mt-4 overflow-x-auto">
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-lg border border-gray-100 shadow-sm text-gray-500">
            Ingresa un ID de vuelo para buscar las reservas.
          </div>
      ...
      <AdminDrawer
        title={selectedReservation ? `Reserva: ${selectedReservation.number}` : "Reserva"}
        isOpen={isDrawerOpen}
        ...
        {selectedReservation && (
          <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg border shadow-sm">
              <h3 className="font-semibold text-gray-800 border-b pb-2 mb-3">Detalles Generales</h3>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-gray-500 font-medium">Vuelo</p>
                  <p className="text-gray-900">{selectedReservation.flight.flightNumber}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Email Contacto</p>
                  <p className="text-gray-900">{selectedReservation.contactEmail}</p>
                </div>
                <div>
                  <p className="text-gray-500 font-medium">Fecha de Reserva</p>
                  <p className="text-gray-900">{new Date(selectedReservation.reservationDatetime).toLocaleString()}</p>
                </div>
                <div>
            ...
            <div className="bg-white p-4 rounded-lg border shadow-sm">
              <h3 className="font-semibold text-gray-800 border-b pb-2 mb-3">Pasajeros</h3>
                  ...
                  {selectedReservation.passengers.map((p, index) => (
                    <div key={index} className="flex justify-between items-center text-sm border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                      <div>
                        ...
                        <p className="text-gray-500 text-xs">Asiento: {p.seatNumber}</p>
                      </div>
                      <span
            ...
            <div className="bg-red-50 p-4 rounded-lg border border-red-200">
              ...
              <p className="text-xs text-red-600 mb-3">
                La cancelación de una reserva eliminará la asignación de asiento y no se puede deshacer.
              </p>
              <button
                onClick={handleCancelReservation}
                disabled={isSubmitting || selectedReservation.status === "CANCELLED"}
